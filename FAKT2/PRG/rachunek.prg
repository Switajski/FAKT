...
DO WHILE .NOT. abbruch
   ...
   m->poz     := poz
   podglad(0)
   @ 5,2  SAY 'L.P. zamowienie/poz ilosc op. Art.Nr     Opis Wyrobu ' COLOR farb4
   ...
   @ 6,22 GET m->ilosc PICTURE '9999' VALID spr_ilosc(m->zamowienie,m->ZAM_POZ,m->ilosc)
   @ 6,27 GET m->karton PICTURE '9999'
   READ
...
DO WHILE wiersz < 23 .AND. .NOT. BOF()
   ...
   @ wiersz,21 SAY STR(ilosc,4,0)
   @ wiersz,26 SAY STR(karton,4,0)
   @ wiersz,32 SAY art_nr COLOR farb4
   @ wiersz,43 SAY LEFT(ALLTRIM(nazwa1)+' '+ALLTRIM(nazwa2)+' '+ALLTRIM(nazwa3),35)
...
@ 6,40 SAY ALLTRIM(Art_Nr)+' otw.:'+ALLTRIM(STR(ilosc-wykonanie,8,0))
